...
ENV        = test
REPO       = 587737487888.dkr.ecr.us-west-2.amazonaws.com/gotenberg
PRODREPO   = 611791207455.dkr.ecr.us-west-2.amazonaws.com/gotenberg
BRANCHNAME = $(shell git branch | grep \* | cut -d ' ' -f2 | sed -e "s/\//-/g")
COMMITSHA  = $(shell git rev-parse --short HEAD)
TIMESTAMP  = $(shell date +%s)
TAG        = $(b)-$(BRANCHNAME)-$(COMMITSHA)-$(TIMESTAMP)
IMAGE      = gotenberg
DOCKERFILE = images/kube/Dockerfile
...
init:
	mkdir -p .deploy/keys
	$(eval USERNAME := $(shell aws configure get test.role_session_name))
	if [[ ! -f .deploy/keys/role-session-name ]]; \
	then \
		echo ${USERNAME} > .deploy/keys/role-session-name; \
	fi
	if [[ ! -f .deploy/keys/deploy-key ]]; \
	then \
		aws-vault exec test -- kubectl get secret bb8-pk --context $(USERNAME)@opensimsim-test.us-west-2.eksctl.io -n oss-tools -o=custom-columns=:data.bb8-pk | base64 --decode > .deploy/keys/deploy-key; \
	fi
...
patch:
	docker build --pull . \
		...
		-t $(REPO):$(TAG)
	docker tag \
		$(REPO):$(TAG) $(REPO):$(PATCHVERSION)
	docker tag \
		$(REPO):$(PATCHVERSION) $(PRODREPO):$(PATCHVERSION)
	docker push \
		$(REPO):$(PATCHVERSION)
	docker push \
		$(PRODREPO):$(PATCHVERSION)
	git tag -a -m "" $(PATCHVERSION) 
